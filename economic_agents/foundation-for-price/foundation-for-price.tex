...
\newcommand{\SflatT}{\Sflat_{\mathrm{agg}}}   % aggregate (min) floor
\newcommand{\SflatI}{\Sflat_{\mathrm{comp}}}  % composite (product) floor
...
\begin{figure*}[!htbp]
    ...
    \caption{\textbf{Cell-Values and Dual Spreads: Value Surface, Two-Value Gap, Spread Comparison, and Information Premium.}
    (\textbf{A})~Three-dimensional surface of the trading value $\ValT_\Ens(C) = \tol - \min_i\beta_i$ and informational value $\ValI_\Ens(C) = \tol - \prod_i\beta_i/\Sigma^{n-1}$ over $(\tol, n)$ with $\bar\beta=2$, $\Sigma=100$.  The two surfaces separate for $n\ge 2$: the upper surface (informational value) lies strictly above the lower (trading value) for all $n\ge 2$, confirming the Two-Value Theorem.
    (\textbf{B})~Information premium $\Prem = \SflatT - \SflatI$ versus $n$ for four agent pools: uniform floors $\beta_i=1$, $\beta_i=2$, geometrically decreasing, and uniformly random from $[0.5,3]$.  All four series are strictly increasing in $n$, confirming \cref{thm:two-value}(c).  The heterogeneous pool (random) achieves the highest premium at every $n$.
    (\textbf{C})~Trading spread $\SpreadT = 2\min_i\beta_i$ and information spread $\SpreadI = 2\SflatI$ versus ensemble size $n$ for a five-agent pool.  $\SpreadT$ remains flat after the first (best) agent joins; $\SpreadI$ decreases with every new agent.  The gap $\SpreadT - \SpreadI$ widens monotonically, confirming \cref{thm:dual-spread}(b)--(c).
    (\textbf{D})~Relative information premium $\Prem/\SflatT$ versus coefficient of variation $\CV$ for $n\in\{2,5,10,20\}$.  At fixed $n$, the relative premium increases with $\CV$ (heterogeneity increases the information advantage).  At fixed $\CV$, it increases with $n$.  The $n=20$, $\CV=1$ point achieves $\Prem/\SflatT\approx 0.99$: 99\% of the trading spread is recovered as information premium.}
...
\begin{enumerate}[label=\textup{(\alph*)}]
\item $\SflatI(\Ens_n) \to 0$ and $\SflatT(\Ens_n) \to \SflatT_\infty
  = \inf_i \Sflat(A_i) \ge 0$ as $n\to\infty$.
\item The purpose cell $C^*_n = B(\MidP_n, r^*_n)$ satisfies $r^*_n \ge
  \SflatT(\Ens_n) \to \SflatT_\infty$, so the equilibrium cell collapses
  toward a point as $\SflatT_\infty \to 0$.
\item The fundamental value $\FV$ exists and is the unique limit of
  ...
  $\SflatT_\infty = 0$, i.e.\ $\inf_i \Sflat(A_i) = 0$).
\item If $\SflatT_\infty > 0$ (even the best agent has positive floor),
  then $C^*_\infty$ is an interval of width $2\SflatT_\infty$: the
...
$\SflatT_\infty = \inf_i \Sflat(A_i) \ge 0$.
...
which converges as the ensemble grows.  When $\SflatT_\infty = 0$, the
spectral radius $\srad_{\Ens_\infty} = 0$ and $C^*_\infty$ collapses to
$\{\FV\}$.

\textit{(d)}  When $\SflatT_\infty > 0$, the minimum cell width at
equilibrium is $2\SflatT_\infty > 0$ by \cref{thm:price-cell}(c).
\end{proof}
...
\begin{figure*}[!htbp]
    ...
    (\textbf{C})~Price cell diameter $2r^*_n = 2\tol(C^*_n)$ versus ensemble size $n$ for three Class~$\mathcal{C}_E$ ensembles.  For all three, $2r^*_n$ decreases toward $2\SflatT_\infty$: the equilibrium cell collapses to the fundamental-value cell.  Faster-decaying composites (lower composite floor) achieve tighter cells at each $n$.
